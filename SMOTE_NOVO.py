...
class DataBalancer:
    ...
    def apply_smote(self):
        # Separar as 5 primeiras colunas
        first_five_columns = self.df.iloc[:, :5]
        remaining_columns = self.df.iloc[:, 5:]
        
        X = remaining_columns.drop(self.target_column, axis=1)
        y = remaining_columns[self.target_column]
        ...
        # Reintegrar as 5 primeiras colunas com os dados balanceados
        first_five_columns_train = first_five_columns.iloc[self.X_train.index].reset_index(drop=True)
        balanced_df = pd.concat([first_five_columns_train, pd.DataFrame(self.X_train_res, columns=self.X_train.columns)], axis=1)
        balanced_df[self.target_column] = self.y_train_res
...
# Uso da classe
file_path = r'C:\\TCC\\Output\\commons-dbutils\\DATASET_INTERO_NORMALIZADO.csv'
target_column = 'TD'
output_file = r'C:\\TCC\\Output\\commons-dbutils\\balanced_data.csv'
